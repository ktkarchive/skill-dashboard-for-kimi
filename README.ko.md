...
커뮤니티 관심도에 따라 기능을 단계적으로 릴리스합니다. ⭐ 별이 5개씩 늘어날 때마다 다음 기능이 추가됩니다:

| 별 개수 | 기능 | 설명 |
|--------|------|------|
| ⭐ 5 | 라이트/다크 모드 토글 | 현재 GitHub 다크 테마와 라이트 테마 전환 |
| ⭐ 10 | 모바일 반응형 레이아웃 | 좁은 화면에서도 사용 가능한 접이식 테이블 및 조정된 레이아웃 |
| ⭐ 15 | 새 스킬 템플릿 생성기 | YAML frontmatter 템플릿으로 새 스킬 디렉토리를 자동 생성하는 "Create Skill" 버튼 |
| ⭐ 20 | 날짜 (JSON/CSV) | 전체 스킬/MCP 테이블을 JSON 또는 CSV로 다운로드하여 백업 또는 외부 처리에 활용 |

기능 요청이 있으면 GitHub 이슈를 열어주세요.
